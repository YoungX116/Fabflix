...
function generateFilterHead(headline, filterCondition) {
	...
	if (filterCondition.hasOwnProperty("title")) {
		headline += 'With Title Matching ' + filterCondition.title + ', ';
	}
	
	if (filterCondition.hasOwnProperty("year")) {
		headline += 'Released ' + filterCondition.year + ', ';
	}
	
	if (filterCondition.hasOwnProperty("rating")) {
		headline += 'Scoring ' + filterCondition.rating + ', ';
	}
	
	if (filterCondition.hasOwnProperty("rating_nr")) {
		headline += filterCondition.rating + ', ';
	}
	
	if (filterCondition.hasOwnProperty("director")) {
		headline += 'Directed By "' + filterCondition.director + '", ';
	}
	...
	if (filterCondition.hasOwnProperty("query")) {
		headline += " Searching For \"" + filterCondition.query + "\"";
	}
